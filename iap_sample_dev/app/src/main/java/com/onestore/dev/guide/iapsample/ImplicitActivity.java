...
public class ImplicitActivity extends FragmentActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		TabHost.OnTabChangeListener tabChangeListener = new TabHost.OnTabChangeListener() {
			...
			public void onTabChanged(String id) {
				...
				ImplicitCommandInBackgroundFragment commandInBg = (ImplicitCommandInBackgroundFragment) m.findFragmentByTag(TAB_LOGIN_VIEW);
				ImplicitCommandFragment command = (ImplicitCommandFragment) m.findFragmentByTag(TAB_COMMAND_VIEW);
				ImplicitPaymentFragment payment = (ImplicitPaymentFragment) m.findFragmentByTag(TAB_PAYMENT_VIEW);
				...
				if (id.equals(TAB_LOGIN_VIEW)) {
					if (commandInBg == null) {
						transction.add(R.id.container, new ImplicitCommandInBackgroundFragment(), TAB_LOGIN_VIEW);
					} else {
					...
					if (command == null) {
						transction.add(R.id.container, new ImplicitCommandFragment(), TAB_COMMAND_VIEW);
					} else {
					...
					if (payment == null) {
						transction.add(R.id.container, new ImplicitPaymentFragment(), TAB_PAYMENT_VIEW);
					} else {
